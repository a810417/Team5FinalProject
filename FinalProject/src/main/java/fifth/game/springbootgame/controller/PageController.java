...
public class PageController {
	...
	// 模糊搜尋
	@GetMapping("/typeMOHU.controller")
	public String processMOHU() {
		return "pages/typeMOHU";
	}
	...
	@GetMapping(path = "/selectMOHU.controller")
	public String processSelectMOHU(@RequestParam(name = "word") String word, Model m) {
		List<Member> mohuList = mService.selectMOHU(word);
		m.addAttribute("mohuList", mohuList);

		return "pages/selectMOHU";
	}
	...
	public String updateMemberAction(@RequestParam("id") Integer id, @RequestParam("account") String account,
		...
		tempMem.setAccount(account);
		tempMem.setPassword(password);
		tempMem.setNickname(nickname);
		tempMem.setEmail(email);
		byte[] pByte = photo.getBytes();
		tempMem.setPhoto(pByte);

		Member newMem = mService.updateMemberById(tempMem, id);

		if (!newMem.equals(null)) {
			return "redirect:/show.controller";
		}
		return "pages/noMember";
